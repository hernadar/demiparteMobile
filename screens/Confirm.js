...
//Función que busca en la DB todas las recomendaciones
    const getRecommendations = async () => {
        try {
          ...
          const response = await fetch('http://192.168.100.2:3001/api/users/recommendation');
...
const changeRecommendation = async (recommendationData) => {
    try {
      ...
      const response = await fetch('http://192.168.100.2:3001/api/users/'+ recommendationData.users_id + '/recommendation/pending',
      {
        method: 'GET',
...
const update= async(e, id) => {
 ...
 setLoading(true)
    ...
    try {
        ...
        const response = await fetch('http://192.168.100.2:3001/api/users/' + userId + '/recommendation/' + recommendationId + '/confirm/' + id,
        {
            method: 'POST',
